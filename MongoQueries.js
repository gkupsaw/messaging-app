...
const authenticateUser = async (username, password) => {
    ...
    await User.findOne({ username })
        ...
        .catch(err => console.error('Error finding user for authentication'.red, err));
    return result;
}

const registerUser = async (username, password) => {
    let result;
    let data = await User.findOne({ username }).catch(err => console.error('Error searching for duplicate user:'.red, err));
    if (data)
        console.log('Found user of same name'.magenta);
    else {  
...
const genPhoneNumber = async () => {
    ...
    while (!uniqueNumber) {
        ...
        allNumbers.forEach(number => { if (number === result) uniqueNumber = false });
    }
...
const getUser = async id => {
    ...
    await User.findOne({ id })
        .then(data => {
            data ? console.log('User located. Sending data...'.green) : console.error('User not found.'.red);
            user = data;
        })
...
const createRoom = async (name, user) => {
    ...
    await newRoom.save();
    await User.updateOne({ id: user.id }, { $push: { rooms: { name: newRoom.name, id: newRoom.id }} })
...
const genFirstRoom = async user => {
    ...
    await firstRoom.save()
        ...
        .catch(err => console.error('Error creating first Room document for new user:'.red, err));
    return firstRoom;
}
...
const getRoom = async id => {
    ...
    await Room.findOne({ id })
        .then(data => {
            data ? console.log('Room located. Sending data...'.green) : console.error('Room not found.'.red);
            room = data;
        })
